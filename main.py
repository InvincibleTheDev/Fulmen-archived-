...
async def save_audit_logs(guild):
  log_channel = discord.utils.get (bot.get_all_channels(), name="staff-logs")
  async for entry in guild.audit_logs(limit=100):
    await log_channel.send('{0.user} did {0.action} to {0.target}'.format(entry))

@bot.command() 
async def remove_features_ping(ctx):
    pings_rank = discord.utils.get(ctx.guild.roles, name="features")
    await bot.remove_roles(pings_rank)
